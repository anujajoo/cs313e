...
def changePi(str):
    if len(str) == 0:
        ...
        num = '3.14'
        recur = changePi(str[2:])
        return "{}{}".format(num, recur)
    else:
...
#return bool bc has to be "if" or not
...
def array11(nums, index):
    ...
    #     return 1 + array11(nums, index + 1)
        ...
        return array11(nums, index + 1) + int(nums[index] == 11)
        #have bool within condition. ture = 1 and false = 0, perf for count
...
def countHi2(str):
    ...
    elif str[0] == 'x' and str[1] == 'h' and str[2] == 'i':
...
def strCount(str, sub):
    ...
    if len(str) == 0:
        return 0
    elif str[:subindex] == sub:
        return 1 + strCount( str[subindex:] , sub)
    else:
