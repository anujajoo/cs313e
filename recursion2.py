...
def groupNoAdj(start, nums, target):
    if start >= len(nums):
        ...
        #returning just target means not choosing, returning target - nums means choosing hence start + 2
...
def groupSumClump(start, nums, target):
    ...
    elif comp < len(nums) and nums[start] == nums[comp]:
        dup = nums[start]
        count = 0
        #goes through loop to see how many duplicates there are and find pos at ending dup
        for pot_duplicate in range (comp + 1, len(nums)):
            if nums[pot_duplicate] == nums[comp]:
                count += 1
            else:
                break
        #wither counts sum of dup in total or doesnt count sum of dup, either way starts next iteration at end of dup pos
        return groupSumClump(comp + count + 1, nums, target - dup*(count+2)) or groupSumClump(comp + count + 1, nums, target)
        
    else:
        return groupSumClump(comp, nums, target) or groupSumClump(comp, nums, target - nums[start])
...
# (No loops needed.)
def splitArray(nums):
#create two lists, return bool at end, and compare sum subtraction across lists
    return splitArrayHelper(nums, 0, 0, 0)
#sum list1 and sum list 2 are same, then true. have to build list while summing.

def splitArrayHelper(nums, sum1, sum2, start):
    if start >= len(nums):
        return sum1 == sum2
    #create binary tree looping through each potential index as add to sum1 or sum2. 
    #therefore it compounds to where sum1 has pos1 comparing against sum2 without pos1 AND sum2 has pos1 comp against sum1 without pos1, etc.
...
def splitOdd10Helper(nums, sum1, sum2, start):
    if start >= len(nums):
        #two ways to meet condition. sum1 10 or sum2 10 with the other list fitting.
        return ((sum1 % 10 == 0) and (sum2 % 2 == 1)) or ((sum2 % 10 == 0) and (sum1 % 2 == 1))
    #creates running binary trees of sums with diff list elements
...
def split53Helper(nums, start, diff):
    ...
    if start >= len(nums):
        #at end of list, is the running diff 0 or not.
        return diff == 0
    
    #assign curr after list check otherwise index error
    curr = nums[start]
    
    if curr % 5 == 0:
        #5 list subtracts
        return split53Helper(nums, start + 1, diff + curr)

    #order matters here bc check 5 first then 3. 15 belongs to 5 list not 3 list
    
    elif curr % 3 == 0:
        #3 list adds
        return split53Helper(nums, start + 1, diff - curr)

    #if neither, can belong to any list. therefore running binary trees of add and subtract
    return split53Helper(nums, start + 1, diff + curr) or split53Helper(nums, start + 1, diff - curr)
